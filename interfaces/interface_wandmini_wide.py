...
def setWideIn(handle):
    success = False
    val, readSuccess = readReg(handle,0,0x0C)
    if not readSuccess:
        logging.error('Failed to read wide input register!')
    else:
        value = val
        ...
        if not (value % 2):
            ...
            value = value + 1
            if (writeReg(handle,0,0x0C,value)):
                logging.error('Wide input mode enabled!')
                success = True
            else:
                logging.error('Unable to write wide input register!')
        else:
            logging.error('Wide input already enabled!')
            success = True
    return success
...
def decodeFn(data: bytes):
    ...
    """raw_bytes = data[2:]  # Skip CRC/status/etc. if needed
    ...
    for ch in channels_selected:
        if 0 <= ch < total_channels:
            ...
            logging.warning(f"Selected channel {ch} out of range")
